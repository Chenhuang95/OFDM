...
positioning accuracy$^{\cite{Qin2019}}$, and the development of RTK
technology improves the positioning accuracy to the centimeter level
with the help of ground-assisted stations$^{\cite{Brack2017Reliable}}$
...
sufficient positioning accuracy. The indoor RFID positioning method$^{\cite{Shangguan2016STPP}}$$^{\cite{Fu2017One}}$
...
learning to locate the channel state information (CSI)$^{\cite{Song2018CSI}}$,
...
anti-multipath performance, OFDM technology can be used to improve
the accuracy of indoor ranging and positioning. In\citep{WangOFDM}\citep{Wang2011Bounds}
, the Cramero Lower Bound (CRLB) based on OFDM was studied, and the
single-path channel was studied in \citep{WangOFDM}, and the performance
of OFDM was limited. In \citep{Wang2011Bounds} , the ranging of the
multipath channel is obtained in the frequency domain. Literature
\citep{Wang70} proved that with the same positioning accuracy, non-data-bearing
orthogonal frequency division multiplexing (NDB OFDM) in wireless
positioning is more energy-efficient than non-pulse-shaping pseudo-random
noise (NPS PN) currently widely used in satellites and the ground.
Articles \citep{DaiDistributed,LiRobust,Shahmansoori2017} improve
positioning accuracy by optimizing network and carrier power allocation.But
the signal power to locate the channel has higher requirements.Many
positioning algorithms are based on ranging information, such as the
least squares algorithm$^{\cite{NguyenLeast}}$ and the chan algorithm$^{\cite{MuUWB}}$.
...
the subcarriers. 
\item we focus on orthogonal frequency- division multiplexing (OFDM) systems
and derive the bounds for OFDM ranging accuracy in multipath channels
based on Fisher information analysis. Prove the EFI of multi-frequency
ranging and how it is affected by the channel parameters. 
...
propagation delay of the pth path, ${\tau_{0}}$ is the number of
arrival paths, and can be expressed as ${\tau_{0}}=R/c$, where $R$
is the distance between the transmitter and the base station, $c$
is the speed of light. Therefore, the estimation of the distance can
be converted into the estimation of the delay. The unknown parameters
...
\end{equation}
In Eq. (1), $i={\rm {1}},{\rm {2}},{\rm {3}}$ represents different
...
\end{aligned}
\end{equation}
In Eq. (2), ${\varphi_{ijk}}$ is the carrier phase combined observation
...
\end{array}}\right]
\end{equation}
It can be known from Eq. (7) that the noise amplification factor in
terms of length is the smallest when the combination coefficient is
proportional to the subcarrier frequency. Therefore, in the OFDM ranging
system, the combination coefficient is determined according to the
predetermined subcarrier frequency to ensure that the result is affected
by noise Minimal, it is helpful to improve the accuracy of the ambiguity
solution. The geometric distance between the transmitting end and
the receiving end is calculated by combining the phase observations.

\section{positioning algorithm and technology}

\begin{figure}[htbp]
\begin{centering}
\textsf{\includegraphics[width=3.5in]{picture/position}} 
\par\end{centering}
\caption{Indoor wireless positioning diagram}
\end{figure}

Figure 5 illustrates the indoor positioning method. The receiver measures
the distances $R_{1}$, $R_{2}$, $R_{3}$, $R_{4}$ and the coordinates
of the base stations from the four base stations, and obtains the
coordinates of the receiver through a geometric positioning algorithm.
Geometric positioning methods, such as the least squares method, are
closely related to positioning accuracy. The next chapter proves the
ranging accuracy of this system.The algorithm can be summarized as
follows. 
\begin{enumerate}
...
time k, the distances between the Tag and the four Anchors are $R_{1},R_{2},R_{3},R_{4}$,
respectively. 

The base station positioning principle is to measure the distance
between a tag and 3 or more Anchors, and solve the distance equation
to get the position coordinates of the measured tag, which can be
expressed as follows: 
\begin{equation}
R\left(x,y,z\right)=\sqrt{\left(x-x_{a}\right)^{2}+\left(y-y_{a}\right)^{2}+\left(z-z_{a}\right)^{2}}
\end{equation}

Among them,$\left(x,y,z\right)$ is the TAG coordinate to be solved,
$\left(x_{a},y_{a},z_{a}\right)$ is the Anchor coordinate calibrated
in advance, and R is the geometric distance between the TAG and Anchor.
...
the first-order Taylor expansion of the ranging Eq.(14) at $\hat{\boldsymbol{\beta}}$
is obtained as:
...
According to Eqs.(14),(15),(16),(17),(18) and the partial differential
...
Write Eq.(16) as a matrix, as follows:
...
Finally, the least square solution of Eq.(17) is obtained as follows:
...
From the above Eq.(15) to (24), the solution process of the nonlinear
...
matrix $\mathbf{H}$ of the ranging direction by using Eqs.(18),(19).

Step 4: Use Eq.(24) to solve the state estimation error vector $\triangle\hat{\boldsymbol{\beta}}$.

Step 5: according to Eq.(17), update the current iteration state:
...
The likelihood function of $\theta$ satisfies the following conditions
...
the FIMs for all subcarriers as shown in Eq. (18).
...
is the power of the $k$th subcarrier. As can be seen from Eqs. (20),
(21) and (22), FIM is independent of the phase of the data. Substitute
...
The approximate EFI for ranging based on Eqs. (24), (25), and (26)
...
equation 21 is the EFI of ranging in the single- path channel, and
the second term represents the effect of path-overlap in multipath
channels.we will quantify the impact of OFDM channel parameters on
EFI based on Eq. (28).We consider total energy constraint, in which
the energy of the FFT part of one OFDM symbol is constrained as 
\begin{equation}
...
on Eqs. (28) and (30), EFI can be rewritten as
